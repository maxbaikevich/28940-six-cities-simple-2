...
import {ExeptionFilterInterface} from './exception-filter.interface.js';
...
@injectable()
export default class ExeptionFilter implements ExeptionFilterInterface {
  constructor(
  ...
  public catch(error: Error, _req: Request, res: Response, next: NextFunction): void {
    if(error instanceof HttpError) {
      return this.handleHttpError(error, _req, res, next);
    }
